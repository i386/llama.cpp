...
bool llama_kv_cache::stage_export_kv_page(
    ...
    const uint32_t strm = seq_to_stream[0];
    if (strm >= v_cells.size()) {
        error = "sequence 0 is mapped to an invalid KV stream";
        return false;
    }
...
bool llama_kv_cache::stage_import_kv_page(
    ...
    seq_rm(0, static_cast<llama_pos>(desc.token_start), static_cast<llama_pos>(desc.token_start + desc.token_count));
