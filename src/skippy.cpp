...
enum skippy_status skippy_model_info_open(
    ...
    *out_info = nullptr;
    ...
    if (!skippy_parse_tensor_metadata(info)) {
        gguf_free(ctx);
        delete info;
        skippy_set_error(out_error, SKIPPY_STATUS_MODEL_ERROR, "failed to parse GGUF tensor descriptors");
        return SKIPPY_STATUS_MODEL_ERROR;
    }
    *out_info = info;
    return skippy_success(out_error);
...
enum skippy_status skippy_write_slice_gguf(
    ...
    std::vector<const skippy_tensor_meta *> selected;
    ...
    if (selected.empty()) {
        skippy_set_error(out_error, SKIPPY_STATUS_INVALID_ARGUMENT, "slice plan selected no tensors");
        return SKIPPY_STATUS_INVALID_ARGUMENT;
    }
    ...
    for (const skippy_tensor_meta * meta : selected) {
    ...
    FILE * input = ggml_fopen(info->path.c_str(), "rb");
    ...
    if (input == nullptr || output == nullptr) {
        ...
        ggml_free(ggml_ctx);
        gguf_free(out_ctx);
        skippy_set_error(out_error, SKIPPY_STATUS_IO_ERROR, "failed to open GGUF files for tensor copy");
        return SKIPPY_STATUS_IO_ERROR;
    }

    const size_t output_alignment = gguf_get_alignment(out_ctx);
    const size_t source_data_offset = gguf_get_data_offset(info->ctx);
    std::vector<uint8_t> buffer(1024 * 1024);
    std::vector<uint8_t> zeroes(output_alignment, 0);
    bool ok = true;

    for (const skippy_tensor_meta * meta : selected) {
        if (std::fseek(input, static_cast<long>(source_data_offset + meta->offset), SEEK_SET) != 0) {
            ok = false;
            break;
        }
        ...
        const size_t padded = skippy_align(static_cast<size_t>(meta->size), output_alignment);
        const size_t padding = padded - static_cast<size_t>(meta->size);
