...
static llama_token skippy_greedy_sample_ith(skippy_session * session, int32_t index) {
    const llama_vocab * vocab = llama_model_get_vocab(session->stage_model->model);
    const int32_t n_vocab = llama_vocab_n_tokens(vocab);
    const float * logits = llama_get_logits_ith(session->ctx, index);
    if (logits == nullptr) {
        return 0;
    }
...
static bool skippy_compute_token_signal(
    ...
    if (session == nullptr || session->ctx == nullptr || session->stage_model == nullptr || session->stage_model->model == nullptr || out_signal == nullptr) {
        return false;
    }
...
enum skippy_status skippy_session_create_from_resident_prefix(
    ...
    *out_session = nullptr;
    ...
    if (static_cast<uint32_t>(cache_seq_id) >= llama_n_seq_max(model->ctx)) {
        skippy_set_error(out_error, SKIPPY_STATUS_UNSUPPORTED, "resident sequence id cannot be decoded by this context");
        return SKIPPY_STATUS_UNSUPPORTED;
    }
    ...
    if (!llama_memory_seq_rm(memory, cache_seq_id, static_cast<llama_pos>(token_count), -1)) {
        skippy_set_error(out_error, SKIPPY_STATUS_RUNTIME_ERROR, "failed to trim borrowed resident sequence");
        return SKIPPY_STATUS_RUNTIME_ERROR;
    }
    ...
    session->stage_model = model;
    session->ctx = model->ctx;
    session->seq_id = cache_seq_id;
    session->checkpoint_seq_id = -1;
    session->n_past = static_cast<int32_t>(token_count);
    session->borrowed_sequence = true;
    session->borrowed_prefix_tokens = static_cast<int32_t>(token_count);
    if (token_count > 0) {
        session->token_history.assign(token_ids, token_ids + token_count);
    }
    model->ctx->synchronize();
